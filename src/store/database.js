...
function requireCollection(value) {
  ...
  if (!/^[a-z][a-z0-9_-]{1,63}$/i.test(collection)) {
    throw new Error("Invalid SQLite collection name.");
  }
  return collection;
}

function encodeRecord(record) {
  return encryptData(JSON.stringify(record));
}

function decodeRecord(row) {
  const value = JSON.parse(decryptData(row.payload));
  if (!value || typeof value !== "object") throw new Error("SQLite record payload is not an object.");
  return value;
}
...
export function openDatabase() {
  ...
  database.exec(`
    ...
    CREATE INDEX IF NOT EXISTS noema_records_collection_created
      ON noema_records (collection, created_at DESC);
    CREATE INDEX IF NOT EXISTS noema_records_collection_updated
      ON noema_records (collection, updated_at DESC);
  `);
  return database;
}
...
export function setMeta(key, value) {
  openDatabase().prepare(`
    INSERT INTO noema_meta (key, value) VALUES (?, ?)
...
export function countRecords(collection) {
  const row = openDatabase()
    .prepare("SELECT COUNT(*) AS count FROM noema_records WHERE collection = ?")
    .get(requireCollection(collection));
  return Number(row?.count || 0);
}

export function listRecords(collection) {
  const rows = openDatabase()
    .prepare("SELECT payload FROM noema_records WHERE collection = ? ORDER BY created_at ASC, id ASC")
    .all(requireCollection(collection));
  return rows.map(decodeRecord);
}

export function getRecord(collection, id) {
  const row = openDatabase()
    .prepare("SELECT payload FROM noema_records WHERE collection = ? AND id = ?")
    .get(requireCollection(collection), String(id));
...
export function upsertRecord(collection, record) {
  ...
  if (!record || typeof record !== "object" || typeof record.id !== "string" || !record.id) {
    throw new Error(`Cannot save invalid ${name} record.`);
  }
  const now = Date.now();
  ...
  `).run(name, record.id, encodeRecord(record), createdAt, updatedAt);
  return record;
}

export function deleteRecord(collection, id) {
  const result = openDatabase()
    .prepare("DELETE FROM noema_records WHERE collection = ? AND id = ?")
    .run(requireCollection(collection), String(id));
