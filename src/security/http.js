...
let globalLogin = { count: 0, since: Date.now() };
...
export function applyClientIp(req, ip) { try { Object.defineProperty(req.socket, "remoteAddress", { configurable: true, value: ip }); } catch {} }
export function secureCookieSuffix() { return config.PUBLIC_BASE_URL.startsWith("https://") ? "; Secure" : ""; }
...
export function redirect(res, location) { res.writeHead(302, { Location: location, "Cache-Control": "no-store" }); res.end(); }
...
export async function readJson(req, maxBytes) {
  try { const raw = await readBody(req, maxBytes); return raw.length ? JSON.parse(raw.toString("utf8")) : {}; }
  catch (error) { if (error.status) throw error; throw Object.assign(new Error("Request body must be valid JSON."), { status: 400 }); }
}
export function setSecurityHeaders(res) {
  const headers = {
    ...
    "Content-Security-Policy": [
      ...
      "font-src 'self' https://fonts.gstatic.com data:", "img-src 'self' data: blob: https:", "media-src 'self' blob:",
      "connect-src 'self' https://nominatim.openstreetmap.org https://www.googleapis.com https://oauth2.googleapis.com",
      "object-src 'none'", "base-uri 'self'", "frame-ancestors 'none'", "form-action 'self' https://accounts.google.com",
    ].join("; "),
  };
  if (config.isProduction && config.PUBLIC_BASE_URL.startsWith("https://")) headers["Strict-Transport-Security"] = "max-age=31536000; includeSubDomains";
  ...
  res.writeHead = (...args) => { for (const [name, value] of Object.entries(headers)) if (!res.hasHeader(name)) res.setHeader(name, value); return original(...args); };
}
...
export function loginStatus(ip) {
  const now = Date.now();
  if (now - globalLogin.since > LOGIN_WINDOW_MS) globalLogin = { count: 0, since: now };
  const record = loginByIp.get(ip);
  if (record && now - record.since > LOGIN_WINDOW_MS) loginByIp.delete(ip);
  const current = loginByIp.get(ip) || { count: 0, since: now };
  return { locked: current.count >= 5 || globalLogin.count >= 50, remaining: Math.max(0, 5 - current.count) };
}
